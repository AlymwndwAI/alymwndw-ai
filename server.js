...
app.post("/add-to-cart", async (req, res) => {
  try {
    ...
    if (!data) return res.status(500).json({ error: "No response from Shopify" });
    if (data.errors && data.errors.length > 0) return res.status(500).json({ error: data.errors[0].message });
    if (!data.data) return res.status(500).json({ error: "No data returned" });
    ...
    if (userErrors && userErrors.length > 0) return res.status(400).json({ error: userErrors[0].message });
  ...
  try {
    ...
    if (productImageUrl && (productImageUrl.startsWith("http://") || productImageUrl.startsWith("https://"))) {
      try {
        ...
        console.log("Image edit failed, falling back: " + editErr.message);
        response = await openai.images.generate({
          model: "gpt-image-1",
          prompt: "Luxury jewelry: " + productHandle.replace(/-/g, " ") + ". " + englishDesc + ". White background, studio lighting, photorealistic, 8K.",
          n: 1,
          size: "1024x1024",
        });
      }
