...
// ========================================
// JEWELRY INTELLIGENCE LAYER
// ========================================

const {
  enrichProduct
} = require("./brain/product-enrichment");
...
function parseVariant(v, images) {
  ...
  if (optionText.includes("rose gold")) {
    ...
    optionText.includes("platinum 950") ||
    optionText.includes("platinum")
  ) {
    metal = "platinum";
  } else if (
    optionText.includes("gold 18k") ||
    optionText.includes("gold 750") ||
    optionText.includes("gold 18")
  ) {
    metal = "gold";
  } else if (
    optionText.includes("silver 925") ||
  ...
  if (
    optionText.includes("royal purple") ||
    optionText.includes("purple")
  ) {

    stoneColor = "purple";

  } else if (
    optionText.includes("red") ||
    optionText.includes("ruby")
  ) {
    ...
    optionText.includes("blue") ||
    optionText.includes("sapphire")
  ) {

    stoneColor = "blue";

  } else if (
    optionText.includes("green") ||
    ...
    optionText.includes("black")
  ) {

    stoneColor = "black";

  } else if (
    optionText.includes("orange")
  ) {
...
function buildAiFeatures(product) {
  ...
  const hasCollection = (keyword) =>
    collectionTitles.some((c) =>
      c.includes(keyword)
    ) ||
    collectionHandles.some((c) =>
      c.includes(keyword)
    );
  ...
  ai.emotionalTriggers =
    [...new Set(ai.emotionalTriggers)];
  ...
  ai.variantStoneSizes =
    [...new Set(ai.variantStoneSizes)];

  ai.diamondShapes =
    [...new Set(ai.diamondShapes)];

  ai.certifications =
    [...new Set(ai.certifications)];
